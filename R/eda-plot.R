...
#' time-locked to stimulus events, separately per event type (condition), with a
#' confidence-interval band. Each event's epoch over `window` is baseline
#' corrected against the pre-event `baseline` interval, epochs are averaged
#' within a condition, and the band is the standard error of the mean scaled to
#' `ci_level` (Boucsein 2012; Bach event-related SCR averaging). The epoching
#' reuses the event onsets from [getEvents()] on the same sample grid as
#' [edaErscr()].
#'
#' @param x A PhysioExperiment object with a skin conductance assay and events.
#' @param event_types Character vector of event types to average and overlay;
#'   `NULL` (default) uses every event type present.
#' @param window Numeric `c(pre, post)` epoch limits in seconds relative to the
#'   event onset (default `c(-1, 6)`).
#' @param baseline Numeric `c(start, end)` pre-event baseline interval in seconds
#'   subtracted from each epoch, or `NULL` for no baseline correction
#'   (default `c(-1, 0)`).
#' @param channel Channel index to average (default 1).
#' @param assay_name Input assay (default: `defaultAssay(x)`).
#' @param ci Confidence band: `"sem"` (mean +/- z * SEM, default) or `"none"`.
#' @param ci_level Confidence level for the SEM band (default 0.95).
#' @param colors Optional per-condition colours (recycled).
#' @param plot Draw the plot (default `TRUE`); set `FALSE` to only compute.
#' @param ... Additional arguments passed to \code{plot()}.
#'
#' @return Invisibly, an `erscr_grand_average` object with `grand_average` (a
#'   tidy data frame of `type`, `time`, `mean`, `sd`, `sem`, `ci_lower`,
#'   `ci_upper`, `n_trials`), the per-condition baseline-corrected `epochs`
#'   matrices, and the `time` axis.
#' @references Boucsein, W. (2012). Electrodermal Activity. 2nd ed. Springer.
#' @seealso [edaErscr()]
...
plotErscrGrandAverage <- function(x, event_types = NULL, window = c(-1, 6),
                                  baseline = c(-1, 0), channel = 1,
                                  assay_name = NULL, ci = c("sem", "none"),
                                  ci_level = 0.95, colors = NULL, plot = TRUE,
                                  ...) {
  stopifnot(inherits(x, "PhysioExperiment"))
  ci <- match.arg(ci)
  if (!is.numeric(window) || length(window) != 2L || any(!is.finite(window)) ||
      window[1] >= window[2]) {
    stop("`window` must be c(pre, post) with pre < post.", call. = FALSE)
  }
  if (!is.null(baseline) && (!is.numeric(baseline) || length(baseline) != 2L ||
      any(!is.finite(baseline)) || baseline[1] >= baseline[2] ||
      baseline[1] < window[1] || baseline[2] > window[2])) {
    stop("`baseline` must be c(start, end) within `window`, or NULL.",
         call. = FALSE)
  }
  stopifnot(is.numeric(ci_level), length(ci_level) == 1L, is.finite(ci_level),
            ci_level > 0, ci_level < 1)

  if (is.null(assay_name)) assay_name <- defaultAssay(x)
  data <- SummarizedExperiment::assay(x, assay_name)
  sr <- samplingRate(x)
  if (!is.numeric(channel) || length(channel) != 1L || !is.finite(channel) ||
      channel != round(channel) || channel < 1L || channel > ncol(data)) {
    stop(sprintf("`channel` must be a single integer index in [1, %d].",
                 ncol(data)), call. = FALSE)
  }
  channel <- as.integer(channel)
  sig <- as.numeric(data[, channel])
  n_time <- length(sig)

  ev <- getEvents(x)
  ev_df <- ev@events
  if (is.null(ev_df) || nrow(ev_df) == 0L) {
    stop("`x` has no events; attach stimulus events with setEvents().",
         call. = FALSE)
  }
  all_types <- unique(as.character(ev_df$type))
  types <- if (is.null(event_types)) all_types else {
    intersect(as.character(event_types), all_types)
  }
  if (!length(types)) {
    stop("none of `event_types` are present in the object's events.",
         call. = FALSE)
  }

  rel <- round(window[1] * sr):round(window[2] * sr)
  epoch_time <- rel / sr
  b_idx <- if (is.null(baseline)) integer(0) else {
    which(epoch_time >= baseline[1] & epoch_time <= baseline[2])
  }
  if (!is.null(baseline) && !length(b_idx)) {
    stop("`baseline` contains no samples on the epoch grid; widen it or raise ",
         "the sampling rate.", call. = FALSE)
  }
  z <- stats::qnorm(1 - (1 - ci_level) / 2)

  epochs_list <- list(); ga_rows <- list()
  for (ty in types) {
    onsets <- as.numeric(ev_df$onset[as.character(ev_df$type) == ty])
    # keep only epochs whose whole window (incl. baseline) is in the recording,
    # so the mean and the CI are over the same, fully-observed set of trials
    eps <- list(); n_drop <- 0L
    for (on in onsets) {
      idx <- (round(on * sr) + 1L) + rel
      if (any(idx < 1L | idx > n_time)) { n_drop <- n_drop + 1L; next }
      seg <- sig[idx]
      if (length(b_idx)) seg <- seg - mean(seg[b_idx])
      eps[[length(eps) + 1L]] <- seg
    }
    if (n_drop > 0L) {
      message(sprintf(paste0("plotErscrGrandAverage: dropped %d '%s' epoch(s) ",
                             "extending beyond the recording."), n_drop, ty))
    }
    ep <- if (length(eps)) {
      do.call(cbind, eps)
    } else {
      matrix(numeric(0), nrow = length(rel), ncol = 0L)
    }
    n <- ncol(ep)
    m <- if (n >= 1L) rowMeans(ep) else rep(NA_real_, length(rel))
    sdw <- if (n >= 2L) apply(ep, 1L, stats::sd) else rep(NA_real_, length(rel))
    sem <- sdw / sqrt(n)
    half <- if (ci == "sem") z * sem else rep(NA_real_, length(m))
    epochs_list[[ty]] <- ep
    ga_rows[[ty]] <- data.frame(
      type = ty, time = epoch_time, mean = m, sd = sdw, sem = sem,
      ci_lower = m - half, ci_upper = m + half, n_trials = n,
      stringsAsFactors = FALSE)
  }
  ga <- do.call(rbind, ga_rows)

  if (isTRUE(plot)) {
    pal <- c("#4C78A8", "#F58518", "#54A24B", "#E45756", "#72B7B2", "#B279A2")
    cols <- rep_len(if (is.null(colors)) pal else colors, length(types))
    yl <- range(c(ga$ci_lower, ga$ci_upper, ga$mean), na.rm = TRUE)
    if (!all(is.finite(yl))) yl <- range(ga$mean, na.rm = TRUE)
    if (!all(is.finite(yl))) yl <- c(-1, 1)          # no complete epochs at all
    # let the caller override the labels/title via `...` without colliding with
    # the structural plot arguments (type/ylim/x/y stay fixed)
    dots <- list(...)
    main <- if (is.null(dots$main)) "ER-SCR grand average" else dots$main
    xlab <- if (is.null(dots$xlab)) "time from event (s)" else dots$xlab
    ylab <- if (is.null(dots$ylab)) "phasic SC (baseline-corrected)" else dots$ylab
    dots[c("main", "xlab", "ylab", "type", "ylim", "x", "y")] <- NULL
    # graphics::plot (not `plot`) -- the `plot` argument shadows the function
    do.call(graphics::plot,
            c(list(epoch_time, ga_rows[[types[1]]]$mean, type = "n", ylim = yl,
                   xlab = xlab, ylab = ylab, main = main), dots))
    abline(v = 0, lty = 3, col = "grey50")
    for (i in seq_along(types)) {
      g <- ga_rows[[types[i]]]
      if (ci != "none" && all(is.finite(c(g$ci_lower, g$ci_upper)))) {
        polygon(c(g$time, rev(g$time)), c(g$ci_lower, rev(g$ci_upper)),
                col = adjustcolor(cols[i], alpha.f = 0.2), border = NA)
      }
      lines(g$time, g$mean, col = cols[i], lwd = 2)
    }
    legend("topright",
           legend = vapply(types, function(t) {
             sprintf("%s (n=%d)", t, ga_rows[[t]]$n_trials[1])
           }, character(1)),
           col = cols, lwd = 2, cex = 0.8)
  }

  out <- list(grand_average = ga, epochs = epochs_list, time = epoch_time,
              window = window, baseline = baseline, ci = ci,
              ci_level = ci_level, channel = channel, assay = assay_name)
  class(out) <- "erscr_grand_average"
  invisible(out)
}

#' @export
print.erscr_grand_average <- function(x, ...) {
  types <- unique(x$grand_average$type)
  cat(sprintf("<erscr_grand_average> %d condition(s), window [%.1f, %.1f] s\n",
              length(types), x$window[1], x$window[2]))
  for (ty in types) {
    g <- x$grand_average[x$grand_average$type == ty, ]
    if (all(is.na(g$mean))) {
      cat(sprintf("  %s: n=%d, no complete epochs\n", ty, g$n_trials[1]))
      next
    }
    pk <- which.max(abs(g$mean))
    cat(sprintf("  %s: n=%d, peak %.3f uS at %.2f s\n",
                ty, g$n_trials[1], g$mean[pk], g$time[pk]))
  }
  invisible(x)
}
