...
edaErscr <- function(x, event_type = NULL, onset_window = c(1, 4),
  ...
  for (ei in seq_len(n_events)) {
    ...
    for (ch in seq_len(n_channels)) {
      ...
      # Walk backward from peak to find SCR onset (local minimum). Guard the
      # boundary case where the peak is at the window start (peak_sample ==
      # s_start): there is nothing to walk back through, and an unguarded
      # seq(peak_sample - 1L, s_start, by = -1L) would have from < to and error.
      onset_sample <- peak_sample
      if (peak_sample > s_start) {
