...
#' @param ns_scr_amplitude_min Minimum SCR amplitude (in uS) to count a peak as
#'   a non-specific SCR (default 0.05, the standard threshold).
...
#'     \item{ns_scr_count}{Integer count of non-specific SCRs (amplitude >=
#'       \code{ns_scr_amplitude_min}).}
#'     \item{ns_scr_freq}{Numeric non-specific SCR frequency (per minute):
#'       \code{ns_scr_count / duration_min}.}
#'     \item{cda_nscr}{Integer number of significant SCRs in the CDA/DDA driver,
#'       or \code{NA} if no driver assay exists.}
#'     \item{cda_ampsum}{Numeric sum of the CDA/DDA driver SCR amplitudes (uS),
#'       or \code{NA}.}
#'     \item{cda_iscr}{Numeric integrated SCR: time-integral of the phasic
#'       driver (`uS*s`), or \code{NA}.}
#'   }
#' @references
...
edaFeatures <- function(x, peaks = NULL, window = NULL, assay_name = NULL,
  ...
  has_driver <- "driver" %in% assay_names
  ...
  driver_data <- if (has_driver) SummarizedExperiment::assay(x, "driver") else NULL
  ...
  if (!is.null(window)) {
    ...
    if (has_driver) {
      driver_data <- driver_data[start_idx:end_idx, , drop = FALSE]
    }
  ...
  for (ch in seq_len(n_channels)) {
    ...
    # Non-specific SCRs: spontaneous SCRs whose amplitude exceeds the standard
    # threshold (default 0.05 uS). Distinct from scr_rate (which counts all peaks).
    ns_scr_count <- if (scr_count > 0) sum(ch_peaks$amplitude >= ns_scr_amplitude_min) else 0L
    ns_scr_freq <- if (duration_min > 0) ns_scr_count / duration_min else 0
    ...
    # CDA/DDA phasic-driver metrics (when a driver assay is present).
    cda_nscr <- NA_integer_
    cda_ampsum <- NA_real_
    cda_iscr <- NA_real_
    if (has_driver) {
      dm <- .cda_metrics(driver_data[, ch], sr)
      cda_nscr <- dm$nSCR
      cda_ampsum <- dm$AmpSum
      cda_iscr <- dm$ISCR
    }
    ...
    results[[ch]] <- data.frame(
      ...
      scl_sd = scl_sd_val,
      auc_phasic = auc,
      ns_scr_count = ns_scr_count,
      ns_scr_freq = ns_scr_freq,
      cda_nscr = cda_nscr,
      cda_ampsum = cda_ampsum,
      cda_iscr = cda_iscr,
